package com.javaedge.java8;

import com.javaedge.java8.collector.MostPopularCollector;
...
public class StreamDetailTest {
    ...
    @Before
    public void data() {
        orders1 = Order.getData();
        orders2 = Order.getData();
    ...
    public void map() {
        ...
        orders1.forEach(tempOrder1 ->
                tempOrder1.getOrderItemList().forEach(orderItem -> longAdder.add(orderItem.getProductQuantity())));

        orders1.stream()
                .map(order1 -> orders2.stream()
                        .filter(order2 -> Objects.equals(order1.getId(), order2.getId()))
                        .findFirst()
                        .map(order2 -> {
                            order1.setCustomerName(order2.getCustomerName());
                            return order1;
                        }).orElse(null))
                .collect(Collectors.toList());

        //使用两次mapToLong+sum方法实现
        assertThat(longAdder.longValue(), is(orders1.stream().mapToLong(order ->
    ...
    public void sorted() {
        ...
        orders1.stream().filter(order -> order.getTotalPrice() > 50)
    ...
    public void flatMap() {
        //不依赖订单上的总价格字段
        System.out.println(orders1.stream().mapToDouble(order -> order.getTotalPrice()).sum());

        //如果不依赖订单上的总价格,可以直接展开订单商品进行价格统计
        System.out.println(orders1.stream()
    ...
    public void groupBy() {
        ...
        System.out.println(orders1.stream().collect(groupingBy(Order::getCustomerName, counting()))
                .entrySet().stream().sorted(Map.Entry.<String, Long>comparingByValue().reversed()).collect(toList()));

        System.out.println("//按照用户名分组,统计订单总金额");
        System.out.println(orders1.stream().collect(groupingBy(Order::getCustomerName, summingDouble(Order::getTotalPrice)))
                .entrySet().stream().sorted(Map.Entry.<String, Double>comparingByValue().reversed()).collect(toList()));

        System.out.println("//按照用户名分组,统计商品采购数量");
        System.out.println(orders1.stream().collect(groupingBy(Order::getCustomerName,
        ...
        orders1.stream().collect(groupingBy(Order::getCustomerName, collectingAndThen(maxBy(comparingDouble(Order::getTotalPrice)), Optional::get)))
                .forEach((k, v) -> System.out.println(k + "#" + v.getTotalPrice() + "@" + v.getPlacedAt()));

        System.out.println("//根据下单年月分组统计订单ID列表");
        System.out.println(orders1.stream().collect
    ...
    @Test
    public void maxMin() {
        orders1.stream().max(comparing(Order::getTotalPrice)).ifPresent(System.out::println);
        orders1.stream().min(comparing(Order::getTotalPrice)).ifPresent(System.out::println);
    }
    ...
    public void distinct() {
        ...
        System.out.println(orders1.stream().map(order -> order.getCustomerName()).collect(joining(",")));

        System.out.println("//去重的下单用户");
        System.out.println(orders1.stream().map(order -> order.getCustomerName()).distinct().collect(joining(",")));
    ...
    public void collect() {
        ...
        System.out.println(orders1.stream().limit(2).collect(toCollection(LinkedList::new)).getClass());
        ...
        System.out.println(orders1.stream().collect(averagingInt(order ->
                order.getOrderItemList().stream()
                        .collect(summingInt(OrderItem::getProductQuantity)))));
    }
    ...
    public void partition() {
        //先来看一下所有下单的用户
        orders1.stream().map(order -> order.getCustomerName()).collect(toSet()).forEach(System.out::println);
        //根据是否有下单记录进行分区
        System.out.println(Customer.getData().stream().collect(
                partitioningBy(customer -> orders1.stream().mapToLong(Order::getCustomerId)
